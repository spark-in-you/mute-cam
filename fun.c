...
int getPidByName(u64 kfd, char* nm) {
    return kread32(kfd, getProcByName(kfd, nm) + 0x8);//PROC_P_PID_OFF); originally 0x60
}
...
//    uint32_t off_vnode_iocount = 0x64;
    uint32_t off_vnode_iocount = 0x3ea;
...
int funCSFlags(u64 kfd, char* process) {
    print("crashes getting pid?");
    uint64_t pid = getPidByName(kfd, process);
    print("crashes getting proc?");
    uint64_t proc = getProc(kfd, pid);
    
    print("crashes getting proc_ro?");
    uint64_t proc_ro = kread64(kfd, proc + 0x18);
    print("crashes getting csflags?");
...
//    print("csflags: launchd (crashes here?)\n");
//    funCSFlags(kfd, "launchd");
//
//    print("funtask: launchd\n");
//    funTask(kfd, "launchd");
//    print("funtask: kfd\n");
//    funTask(kfd, "kfd");
//    print("funtask: springboard\n");
